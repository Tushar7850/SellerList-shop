...
function Checkoutpage() {
   const {cartItems,CarttotalPrice,TotalShippingChrges}= useContext(CardContext)
...
<div className="grid sm:px-10 lg:grid-cols-2 lg:px-20 xl:px-32">
    ...
    <div className="mt-8 space-y-3 h-64 overflow-y-scroll rounded-lg border bg-white px-2 py-4 sm:px-6">

        {
          cartItems.map((product)=>(
            ...
            <img className="m-2 h-24 w-28 rounded-md border object-cover object-center" src={product?.main_img} alt="" />
  ...
  <div className="mt-10 bg-gray-50 px-4 pt-8 lg:mt-0">
    ...
    <div className="">
      ...
      <div className="relative">
        <input type="text" id="email" name="email" value={user?.email}disabled='true' className="w-full rounded-md border border-gray-200 px-4 py-3 pl-11 text-sm shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500" placeholder="your.email@gmail.com" />
      ...
      <div className="mt-6 border-t border-b py-2">
        ...
        <div className="flex items-center justify-between">
          ...
          <p className="font-semibold text-gray-900">₹{ TotalShippingChrges}</p>
        </div>
      </div>
      <div className="mt-6 flex items-center justify-between">
        <p className="text-sm font-medium text-gray-900">Total</p>
        <p className="text-2xl font-semibold text-gray-900">₹{CarttotalPrice + TotalShippingChrges}</p>
      </div>
    </div>
